...
namespace CAD{
    namespace gui{
        ...
        private:
            ...
            char renameBuff[64];
        public:
            EngineGUI(std::string name, std::string id, int width, int height, std::array<NRA::VGL::ControlBind, 17> controls, NRA::VGL::Shader &shader, std::size_t promptSize = 255, std::size_t promptHistoryCount = 100);
            ~EngineGUI(){};
